...
// ===== YAVER FIX: make sure top.$ is jQuery (some TW layouts don't expose jQuery on top) =====
try {
    if (window.top && window.$ && window.$.getScript) {
        if (!window.top.$ || !window.top.$.getScript) {
            window.top.$ = window.$;
        }
        if (!window.top.jQuery && window.jQuery) {
            window.top.jQuery = window.jQuery;
        }
    }
} catch (e) {}
...
function yaver_patch_inline_onclicks() {
    try {
        var $root = window.top.$("#settingsDiv");
        if (!$root.length) return;

        $root.find("[onclick]").each(function () {
            var v = this.getAttribute("onclick");
            if (!v) return;

            v = v
              .replace(/\bapplySettings\s*\(/g, "window.applySettings(")
              .replace(/\bresetTable\s*\(/g, "window.resetTable(")
              .replace(/\bchangeProfile\s*\(/g, "window.changeProfile(")
              .replace(/\bcreateProfile\s*\(/g, "window.createProfile(")
              .replace(/\bsetDefaultProfile\s*\(/g, "window.setDefaultProfile(")
              .replace(/\bdeleteProfile\s*\(/g, "window.deleteProfile(")
              .replace(/\bupdateProfile\s*\(/g, "window.updateProfile(")
              .replace(/\bexportProfile\s*\(/g, "window.exportProfile(")
              .replace(/\bimportProfile\s*\(/g, "window.importProfile(")
              .replace(/\bloadLanguage\s*\(/g, "window.loadLanguage(")
              .replace(/\buglyHider\s*\(/g, "window.uglyHider(")
              .replace(/\bsetKeyEditMode\s*\(/g, "window.setKeyEditMode(");

            this.setAttribute("onclick", v);
        });
    } catch (e) {}
}
...
function applySettings() {
    try { if (window.updateProfile) window.updateProfile(); } catch(e) {}
    if (!pagesLoaded) {
...
// ===== YAVER FIX: guaranteed updateProfile() (bundler can't drop this because it's attached to window) =====
function yaver_updateProfile_proxy() {
  try {
    var $ = window.top.$ || window.$;
    if (!$ || !$.jStorage) {
      console.log("[Yaver] jStorage not ready yet");
      return;
    }
    ...
    userset = settings; // scriptin kullandığı aktif ayarlar
    if (window.top.UI && window.top.UI.SuccessMessage) window.top.UI.SuccessMessage("Saved ✅", 700);
  } catch (e) {
    console.log("[Yaver] updateProfile proxy failed:", e);
  }
}

// export it (this is the important part)
try {
  window.updateProfile = yaver_updateProfile_proxy;
  window.top.updateProfile = yaver_updateProfile_proxy;
} catch(e) {}


// ===== YAVER FIX: expose functions globally for inline onclick handlers =====
// ===== YAVER FIX: export functions for inline onclick handlers (WB$wombat wrapper hides them) =====
try {
  ...
  // optional (profile buttons)
  window.createProfile = createProfile;
  window.top.createProfile = createProfile;

  window.setDefaultProfile = setDefaultProfile;
  window.top.setDefaultProfile = setDefaultProfile;

  window.deleteProfile = deleteProfile;
  window.top.deleteProfile = deleteProfile;
// [Yaver] removed: window.updateProfile = updateProfile; (it breaks proxy)
// [Yaver] removed: window.top.updateProfile = updateProfile; (it breaks proxy)
  window.exportProfile = exportProfile;
  window.top.exportProfile = exportProfile;

  window.importProfile = importProfile;
  window.top.importProfile = importProfile;

  window.__YAVER_EXPORT_OK__ = true;
} catch (e) {
  console.log("[Yaver] export failed", e);
}
...
// ===== YAVER OUTSIDE PATCH (last-wins) =====
window.__YAVER_UPROXY__ = "v2-uproxy-001";

// Create a stable updateProfile that ALWAYS exists for inline onclick handlers.
// This writes settings into jStorage AND updates userset so filters work immediately.
(function () {
  function pickJQ() {
    // Prefer the game's jQuery (top), then local.
    try {
  ...
  window.updateProfile = function yaver_updateProfile_proxy_global() {
    try {
      var $ = pickJQ();
      if (!$ || !$.jStorage) {
        console.log("[Yaver] jQuery/jStorage not ready yet");
        return;
      }
      ...
      // Try to update in-memory settings used by filters.
      try { window.userset = settings; } catch(e) {}
      try { userset = settings; } catch(e) {}

      if (window.top && window.top.UI && window.top.UI.SuccessMessage) {
        window.top.UI.SuccessMessage("Saved ✅", 700);
      }
    } catch (e) {
      console.log("[Yaver] updateProfile proxy failed:", e);
    }
  ...
  try { window.top.updateProfile = window.updateProfile; } catch(e) {}
  window.__YAVER_EXPORT_OK__ = true;
})();
